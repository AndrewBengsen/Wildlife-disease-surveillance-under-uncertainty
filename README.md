...
## REQUIRED INPUTS 
   - FMDV_risk_3308_v3: polygon layer (.shp) of the hexagonal risk grid, with fields LS_pig, LS_tt_c, LS_tot, cell (see Table 1 of the manuscript for the component risk layers this is built from)
   - synthetic_pigs_3308.RDS: sf point object of synthetic pig sample locations, EPSG:3308, with fields x, y, operation
   - error_df.csv: lookup table of expected detection probability (detect_prob) by sample size (n), from the two-stage sampling model (see Supplementary Figure S2)
...
## KEY PARAMETERS 
   ...
   - eps/minpts: DBSCAN neighbourhood radius (10 km) and minimum cluster size (5 pigs)
   - a, b: weights balancing search value vs. spatial dispersion in cell selection (1, 5)

## SOFTWARE
Produced using R v4.3.1 (R Core Team 2024).  
See `BST_DBSCA_synthetic_example_v1-1.R` for full session info. 

## LICENCE
The accompanying manuscript is licensed under `CC BY-NC-ND 4.0`. Source code and datasets are licensed under the MIT License.  